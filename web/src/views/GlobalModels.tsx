  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-zinc-900 mb-2">Global Models Database</h1>
          <p className="text-zinc-500">
            Source of truth for all LLM baseline metadata and official pricing.
          </p>
        </div>
        <button 
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              <p className="text-sm text-zinc-500">
                Fetch latest official pricing from an external JSON registry url.
              </p>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4 md:col-span-2">
                <div>
                  <label className="block text-xs font-semibold text-zinc-400 uppercase mb-1.5">Model ID (read-only)</label>
                  <div className="bg-zinc-100 text-zinc-500 px-3 py-2 rounded-lg font-mono text-sm border border-zinc-200">{editForm.id}</div>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-zinc-400 uppercase mb-1.5">Display Name</label>
                  <input 
              ...
              <div className="space-y-4">
                <h4 className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Main Pricing ($ / 1M tokens)</h4>
                <div className="space-y-3">
                  <div>
                    <label className="block text-xs text-zinc-500 mb-1">Input Price</label>
                    <input 
                  ...
                  <div>
                    <label className="block text-xs text-zinc-500 mb-1">Output Price</label>
                    <input 
                      type="number" step="0.000001" 
                      className="w-full bg-zinc-50 border border-zinc-200 rounded-lg px-3 py-2 text-sm" 
                      value={ep.completion || ''} 
              ...
              <div className="space-y-4">
                <h4 className="text-xs font-bold text-zinc-400 uppercase tracking-wider">Advanced Pricing</h4>
                <div className="space-y-3">
                  <div className="flex gap-4">
                    <div className="flex-1">
                      <label className="block text-xs text-zinc-500 mb-1">Cache Read</label>
                      <input 
                        type="number" step="0.000001" 
                        className="w-full bg-zinc-50 border border-zinc-200 rounded-lg px-3 py-2 text-sm" 
                        value={ep.cache_read || ''} 
                    ...
                    <div className="flex-1">
                      <label className="block text-xs text-zinc-500 mb-1">Cache Write</label>
                      <input 
                        type="number" step="0.000001" 
                        className="w-full bg-zinc-50 border border-zinc-200 rounded-lg px-3 py-2 text-sm" 
                  ...
                  <div>
                    <label className="block text-xs text-zinc-500 mb-1">Reasoning (per TKN)</label>
                    <input 
                      type="number" step="0.000001" 
                      className="w-full bg-zinc-50 border border-zinc-200 rounded-lg px-3 py-2 text-sm" 
                      value={ep.reasoning || ''} 
      ...
      <div className="bg-white border text-sm border-gray-100 rounded-xl shadow-sm overflow-hidden">
        <table className="w-full text-left border-collapse">
          <thead className="bg-zinc-50/50 border-b border-gray-100">
            <tr>
              <th className="px-6 py-4 font-semibold text-zinc-600">ID / Name</th>
              <th className="px-6 py-4 font-semibold text-zinc-600">Input / Output</th>
              <th className="px-6 py-4 font-semibold text-zinc-600">Cache (R / W)</th>
              <th className="px-6 py-4 font-semibold text-zinc-600">Reasoning</th>
              <th className="px-6 py-4 font-semibold text-right text-zinc-600">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {loading ? (
              <tr><td colSpan={5} className="px-6 py-12 text-center text-zinc-400">Loading models registry...</td></tr>
            ) : filteredModels.length === 0 ? (
              <tr><td colSpan={5} className="px-6 py-12 text-center text-zinc-400">No models found in database.</td></tr>
              ...
              return (
                <tr key={m.id} className="hover:bg-zinc-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-5">
                    <div className="font-mono text-zinc-600 font-medium">${pricing.prompt ?? '-'} / ${pricing.completion ?? '-'}</div>
                  </td>
                  <td className="px-6 py-5">
                    {(pricing.cache_read || pricing.cache_write) ? (
                      <div className="font-mono text-emerald-600 font-semibold">${pricing.cache_read ?? '-'} / ${pricing.cache_write ?? '-'}</div>
                    ) : <span className="text-zinc-300 font-light italic">not cached</span>}
                  </td>
                  <td className="px-6 py-5">
                    {pricing.reasoning ? (
                      <div className="font-mono text-indigo-600 font-semibold">${pricing.reasoning}</div>
                    ) : <span className="text-zinc-300 font-light italic">none</span>}
                  </td>
